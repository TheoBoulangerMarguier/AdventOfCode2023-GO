...
func d13p1() int {
	puzzles := loadDataFromInput("./Ressources/day13_input.txt")
	total := 0
...
func d13p2() int {
	...
	for i := 0; i < len(puzzles); i++ {
		...
		for y := 0; y < len(p.cells); y++ {
			for x := 0; x < len(p.cells[y]); x++ {
				...
				if len(dH) != 0 || len(dV) != 0 {
					...
					p.vertSym = dV
					foundSmudge = true
					fmt.Println("puzzle n°", i+1, "found smudge at", x, y, "new H:", dH, " new V:", dV)
					break
				}
			}
			if foundSmudge {
		...
		if !foundSmudge {
			fmt.Println("puzzle n°", i+1, "didn't find a smudge")
		}

		if len(p.horitzSym) == 1 {
