...
to a point on the side of yet another mountain - the largest mountain on Lava Island.
...
and next to a smaller sign that says "Danger - Personal Protective Equipment
...
Lava Production Facility online - all without burning or melting anything unintended!"

"Before you begin, please be prepared to use the Holiday ASCII String Helper
algorithm (appendix 1A)." You turn to appendix 1A. The reindeer leans closer with interest.

The HASH algorithm is a way to turn any string of characters into a single
number in the range 0 to 255. To run the HASH algorithm on a string, s
tart with a current value of 0. Then, for each character in the string
...
parsing the initialization sequence. To verify that your HASH algorithm is working,
the book offers the sum of the result of running the HASH algorithm on each
...
the reindeer has stolen the page containing the expected verification number
...
positioned to focus light passing through the box. The side of each box has a panel
...
their order, filling any space made by removing the indicated lens.
...
lenses are shown here. Within each box, lenses are listed from front to back;
...
func d15p2() int {
	...
	for _, str := range strs {
		//step 1 convert string to lense
		newLense := Lense{}
		powerStr := ""
		isPart1 := true
		isEqual := false
		for _, r := range str {
			if r == '-' {
				newLense.action = false
				isPart1 = false
				continue
			} else if r == '=' {
				isEqual = true
				newLense.action = true
				isPart1 = false
				continue
			}

			if isPart1 {
				newLense.label += string(r)
			} else if isEqual {
				powerStr += string(r)
			}
		}
		if isEqual {
			v, err := strconv.Atoi(powerStr)
			if err != nil {
				log.Fatal(err)
			}
			newLense.power = v
		...
		if newLense.action {
			...
			if _, ok := boxes[boxID]; ok {
				...
				pos := -1
				for i, l := range boxes[boxID].lenses {
					if l.label == newLense.label {
						pos = i
						break
					}
				}
				if pos != -1 {
					//replace the lense in position
					boxes[boxID].lenses[pos].label = newLense.label
					boxes[boxID].lenses[pos].power = newLense.power
					boxes[boxID].lenses[pos].action = newLense.action
				} else {
					//add the lense
					boxUpdate := Box{
						lenses: append(boxes[boxID].lenses, newLense),
					}
					boxes[boxID] = boxUpdate
				}
			} else {
				//create a new box with the lense in it
				boxes[boxID] = Box{
					lenses: []Lense{newLense},
				}
			}
		} else {
			//REMOVE
			if _, ok := boxes[boxID]; ok {
				...
				if pos != -1 {
					newLenses := append(boxes[boxID].lenses[:pos], boxes[boxID].lenses[pos+1:]...)
					boxUpdated := Box{
						lenses: newLenses,
					}
					boxes[boxID] = boxUpdated
				}
			}
		}
