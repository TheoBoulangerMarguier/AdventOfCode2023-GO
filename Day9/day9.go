...
new sequence from the difference at each step of your history.
...
sequence because at each step it considers two numbers from the input.
Since these values aren't all zero, repeat the process: the values differ
by 0 at each step, so the next sequence is 0 0 0 0. This means you have enough
...
above them, this also means there is now a placeholder in every sequence above it:
...
of -3 for the first history and 0 for the second history.
...
func HistoryDiff(row []int) []int {
	diff := make([]int, len(row)-1)
...
func d9p1() int {
	...
	file, err := os.Open("./Day9/Ressources/day9_input.txt")
	if err != nil {
		log.Fatal(err)
	}
	...
	for scanner.Scan() {
		row, err := utils.AtoiArray(strings.Split(scanner.Text(), " "))
		if err != nil {
			log.Fatal(err)
		}
		...
		for i := 0; i < len(rows); i++ {
			...
			//get the substraction of current history
			subRow := HistoryDiff(rows[i])
			rows = append(rows, subRow)

			//check if new history is all 0
			for j := 0; j < len(subRow); j++ {
				sumDiff += subRow[j]
				if sumDiff != 0 {
					break
				}
			...
			//found history with only 0 so now we add 0 and extrapolate to teh top
			if sumDiff == 0 {
				rows[len(rows)-1] = append(rows[len(rows)-1], 0)

				for j := len(rows) - 1; j > 0; j-- {
					currentLastValue := rows[j][len(rows[j])-1]
					nextLastValue := rows[j-1][len(rows[j-1])-1]
					rows[j-1] = append(rows[j-1], nextLastValue+currentLastValue)
				}
